      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="relative w-full md:w-72">
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-brand-peach focus:outline-none focus:ring-2 focus:ring-brand-orange bg-brand-marfil text-brand-dark text-sm"
              />
        ...
        {filteredCats.length === 0 ? (
          <div className="text-center py-16 bg-brand-bone rounded-3xl border border-dashed border-brand-peach">
            ...
            <p className="text-lg font-bold text-brand-dark">No encontramos michis con esos filtros.</p>
            <p className="text-sm text-brand-dark/70 mt-1">Prueba con otra palabra clave o restablece los filtros.</p>
            <button
          ...
          <>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {cat.personality.slice(0, 3).map((p, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-brand-vanilla/60 text-brand-coffee border border-brand-peach/70"
                        >
                          {p}
                        </span>
                      ))}
            ...
            <div className="mt-12 text-center">
              <button
                ...
                className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl text-base sm:text-lg font-black text-white bg-gradient-to-r from-brand-orange via-orange-600 to-brand-coffee hover:from-orange-600 hover:to-brand-coffee shadow-warm hover:shadow-warm-lg transition-all transform hover:-translate-y-0.5 group cursor-pointer"
              >
                <PawPrint className="w-5 h-5" />
                <span>Ver todos los gatitos ({catsData.length} en adopción)</span>
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </button>
              <p className="text-xs text-brand-dark/70 mt-2 font-medium">
        ...
        <div className="mt-16 bg-brand-bone border border-brand-peach rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-center gap-6 shadow-xs">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-brand-orange to-brand-coffee text-white flex items-center justify-center shrink-0 shadow-md">
          ...
          <a
            href="#faq"
            className="shrink-0 px-5 py-2.5 rounded-xl bg-brand-marfil hover:bg-brand-vanilla/60 text-brand-coffee text-xs sm:text-sm font-bold border border-brand-peach transition-colors shadow-xs"
          >
        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {adoptionSteps.map((step, idx) => (
              ...
              >
                <span className="text-3xl font-black text-brand-peach group-hover:text-brand-orange transition-colors font-display block mb-2">
                  {step.number}
                </span>
