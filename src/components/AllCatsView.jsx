    ...
    <div className="min-h-screen bg-brand-marfil pt-6 pb-20 animate-fadeIn">
      ...
      <div className="sticky top-0 z-30 bg-brand-marfil/95 backdrop-blur-md border-b border-brand-peach/60 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold text-brand-dark hover:text-brand-orange bg-brand-bone hover:bg-brand-peach/60 border border-brand-peach transition-all shadow-xs"
          >
          ...
          <div className="flex items-center gap-2">
            <span className="hidden sm:inline text-xs font-bold text-brand-coffee">
              Censo de Adopción Misicha:
            </span>
            <span className="px-3 py-1 rounded-full text-xs font-black bg-gradient-to-r from-brand-orange to-brand-coffee text-white shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-10">
          <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-brand-bone border border-brand-peach text-brand-coffee text-xs font-black uppercase tracking-wider">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all flex items-center gap-1.5 ${
                    selectedCategory === cat.id
                      ...
                      : 'bg-brand-marfil text-brand-coffee hover:bg-brand-vanilla/60 border border-brand-peach/80'
                  }`}
                >
                  <span>{cat.label}</span>
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-black ${
                    selectedCategory === cat.id ? 'bg-white/25 text-white' : 'bg-brand-bone text-brand-coffee'
                  }`}>
                    {cat.count}
                  </span>
                </button>
            ...
            <div className="relative w-full md:w-80">
              <input
                ...
                className="w-full pl-10 pr-9 py-2.5 rounded-xl border border-brand-peach focus:outline-none focus:ring-2 focus:ring-brand-orange bg-brand-marfil text-brand-dark text-xs sm:text-sm"
              />
              <Search className="w-4 h-4 text-brand-coffee/60 absolute left-3.5 top-3.5" />
              {searchQuery && (
                <button
        ...
        {filteredCats.length === 0 ? (
          <div className="text-center py-16 bg-brand-bone rounded-3xl border border-dashed border-brand-peach shadow-xs">
            ...
            <p className="text-lg font-bold text-brand-dark">No encontramos michis con esos criterios de búsqueda.</p>
            <p className="text-sm text-brand-dark/70 mt-1">Prueba con otro término o limpia los filtros.</p>
            <button
              ...
              >
                ...
                <div className="relative h-64 overflow-hidden bg-stone-100">
                  ...
                  <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-1">
                    <span className="bg-brand-marfil/95 backdrop-blur-xs text-brand-dark text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider shadow-xs">
                      {cat.gender} • {cat.age}
                    </span>
                    {cat.urgent ? (
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-3.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {cat.personality.slice(0, 3).map((p, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-brand-vanilla/60 text-brand-coffee border border-brand-peach/70"
                      >
                        {p}
                      </span>
                    ))}
                  ...
                  {cat.observations && (
                    <div className="bg-brand-bone border border-brand-peach/80 p-2 rounded-xl text-[11px] text-brand-coffee font-medium">
                      <strong>Rasgo distintivo:</strong> {cat.observations}
                    </div>
                  )}

                  {/* Short story preview */}
                  <p className="text-xs text-brand-dark/80 line-clamp-2 leading-relaxed">
                    {cat.history}
                  </p>

                  {/* Health Guarantees */}
                  <div className="pt-2 border-t border-brand-peach/40 flex items-center justify-between text-[11px] text-brand-coffee/80 font-medium">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-brand-orange to-michi-600 hover:from-michi-600 hover:to-brand-coffee shadow-xs hover:shadow-warm transition-all flex items-center justify-center gap-1.5"
                  >
        ...
        <div className="mt-16 text-center">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-bold text-brand-dark hover:text-brand-orange bg-brand-bone hover:bg-brand-peach/60 border border-brand-peach shadow-xs transition-colors"
          >
            <ArrowLeft className="w-4 h-4" />
            <span>Volver a la Página Principal</span>
          </button>
